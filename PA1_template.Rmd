...
for (i in 1:nrow(activity)) {
    ...
    if(is.na(activity$steps[i])) {
        ...
        activity.imp$steps[i] <- avgstepsperinterval$steps[which(avgstepsperinterval$interval==activity$interval[i])]
    }
}
```
...
```{r histogram2}
# create a data frame containing the sum of steps for each date
totalstepsperday.imp <- data.frame(tapply(activity.imp$steps, activity.imp$date, sum, na.rm=TRUE))
...
```{r}
# use wday function from lubridate to determine if each date is weekday or weekend
library(lubridate)
activity.imp$daytype <- as.factor(ifelse(wday(activity.imp$date, label=TRUE) %in% c("Sat", "Sun"), "weekend", "weekday"))
```
